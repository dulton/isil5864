...
#define FILESTORENAME "./test_main.h264"
...
static void* file_store_thread( void * arg)
{
    ...
    ret = ISIL_MediaSDK_RegH264MainCallback( 0 ,channel_num , GetH264CallBack , NULL);
    if( ret < 0 ) {
    ...
    ret = ISIL_MediaSDK_StartH264MainChannel(0 , channel_num );
    if( ret < 0 ) {
    ...
    ISIL_MediaSDK_StopH264MainChannel( 0 , channel_num);
