...
#define FILESTORENAME "./test.mjpeg"
...
static void* file_store_thread( void * arg)
{
    ...
    ret = ISIL_MediaSDK_RegMJpegCallback( 0 ,channel_num , GetH264CallBack , NULL);
    if( ret < 0 ) {
    ...
    ret = ISIL_MediaSDK_StartCapMjpeg(0 , channel_num );
    if( ret < 0 ) {
    ...
    ISIL_MediaSDK_StopCapMjpeg( 0 , channel_num);
