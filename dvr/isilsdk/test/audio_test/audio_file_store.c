...
#define FILESTORENAME "./test.adpcm"
...
static void* file_store_thread( void * arg)
{
    ...
    ret = ISIL_MediaSDK_RegAudioCallback( 0 ,channel_num , GetH264CallBack , NULL);
    if( ret < 0 ) {
    ...
    ret = ISIL_MediaSDK_StartAudioChannel(0 , channel_num );
    if( ret < 0 ) {
    ...
    ISIL_MediaSDK_StopAudioChannel( 0 , channel_num);
